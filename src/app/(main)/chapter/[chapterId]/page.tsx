...
const MangaChapterPage: React.FC = () => {
  ...
  // Use the chapter list hook
  const { chapterListData, loading: chaptersLoading } = useChapterList(
    mangaId || ""
  );
  ...
  useEffect(() => {
    if (chapterListData.length > 0 && chapterId) {
      const index = chapterListData.findIndex(
  ...
  const currentChapter = chapterListData.find(
    (chapter) => chapter.id === chapterId
  );
  ...
  const goToNextChapter = () => {
    if (
      ...
      router.push(`/chapter/${chapterListData[currentChapterIndex + 1].id}`);
    }
  };
